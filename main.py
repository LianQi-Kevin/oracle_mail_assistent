...
MAIN_RE = re.compile(r'^[ \t]*'  # ← 行首空白
                     r'(?:最终[ \t]*)?'  # ← ① 可选“最终”
                     r'(?:\((?P<wf>[A-Za-z]+-\d+)\)[ \t]*)?'  # ← ② 可选 (WF-001039)
                     r'SLDS-BCEG-'  # 固定前缀
                     r'(?P<unit>\d{3})-'  # 单体
                     r'SDS-'
                     r'(?P<discipline>[A-Z]+)-'  # 专业
                     r'(?P<drawing>[A-Z0-9]+)'  # 图纸号
                     r'(?:_*(?P<ver>[A-Z]|\d+\+[A-Z]|\d+[A-Z]|\d+))?'  # 版本号
                     r'(?:[ \t]+(?P<title>.+))?'  # 图名
                     r'[ \t]*$'  # 行尾空白
                     )
            ...
            if not newest_matched_data['ver'].isdigit() and newest_matched_data['wf']:
                ...
                for workflow in workflows_data.workflows:
                    ...
                    if workflow.step_outcome == "正等待处理":
                        ...
                        base_col += 3
            elif newest_matched_data["var"].isdigit():
                for i in range(8):
                    row[i].fill = openpyxl.styles.PatternFill("solid", fgColor="C6EFCE")    # green
